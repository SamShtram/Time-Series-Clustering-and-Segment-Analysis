...
def similarity(ts1, ts2, method="dtw"):
    ...
    if method == "correlation":
        ...
        return 1 - corr  # smaller = higher correlation
...
def select_pivot(segments):
    """Select a pivot signal (mean or random)."""
    idx = np.random.randint(0, len(segments))
    return segments[idx]

def divide_and_conquer_cluster(segments, depth=0, max_size=10, method="dtw"):
    """
    Recursively partition segments into clusters based on similarity.
    """
    if len(segments) <= max_size:
        return [segments]

    pivot = select_pivot(segments)
    distances = [similarity(pivot, s, method) for s in tqdm(segments, disable=(depth > 1))]
